...
function isFileAllowed(filePath) {
  ...
  // 检查是否匹配禁止的模式
  const isForbidden = FORBIDDEN_PATTERNS.some(pattern => pattern.test(filePath));
    ...
    lines.forEach((line, index) => {
      if (line.includes('eslint-disable')) {
        disableComments.push({
          line: index + 1,
          content: line.trim(),
          hasDocumentation: lines[index + 1] && lines[index + 1].includes('/**')
        });
      }
    });
...
function main() {
  ...
  console.log(`📋 发现 ${filesWithDisable.length} 个文件使用了 ESLint 禁用注释:\n`);
  ...
  filesWithDisable.forEach(filePath => {
    ...
    if (isAllowed) {
      ...
      // 检查是否有文档说明
      const undocumented = disableComments.filter(comment => !comment.hasDocumentation);
