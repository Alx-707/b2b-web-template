...
function generateReport() {
  ...
  console.log(colors.bold('\n📊 总体进展'));
  console.log(
    `起始覆盖率: ${colors.cyan(progress.startCoverage.toFixed(2) + '%')}`,
  );
  console.log(
    `当前覆盖率: ${colors.green(progress.currentCoverage.toFixed(2) + '%')}`,
  );
  console.log(
    `目标覆盖率: ${colors.blue(progress.targetCoverage.toFixed(2) + '%')}`,
  );
  console.log(
    `已提升: ${colors.green('+' + progress.currentImprovement.toFixed(2) + '%')}`,
  );
  console.log(
    `还需提升: ${colors.yellow('+' + progress.remainingImprovement.toFixed(2) + '%')}`,
  );
  console.log(
    `完成度: ${colors.cyan(progress.progressPercentage.toFixed(1) + '%')}`,
  );
  ...
  console.log(
    `已覆盖: ${colors.green(analysis.coveredLines.toLocaleString())} 行`,
  );
  console.log(
    `未覆盖: ${colors.red(analysis.uncoveredLines.toLocaleString())} 行`,
  );
  console.log(
    `函数覆盖率: ${colors.cyan(analysis.functions.pct.toFixed(2) + '%')} (${analysis.functions.covered}/${analysis.functions.total})`,
  );
  console.log(
    `分支覆盖率: ${colors.cyan(analysis.branches.pct.toFixed(2) + '%')} (${analysis.branches.covered}/${analysis.branches.total})`,
  );
  ...
  if (keyFiles.length > 0) {
    keyFiles.forEach((file) => {
      ...
      console.log(
        `${status} ${fileName}: ${colors.cyan(file.coverage.toFixed(1) + '%')} (${file.covered}/${file.total})`,
      );
    });
  } else {
  ...
  if (progress.currentImprovement > 0) {
    achievements.push(
      `✅ 成功提升覆盖率 ${progress.currentImprovement.toFixed(2)}%`,
    );
  }
  ...
  if (progress.remainingImprovement > 0) {
    console.log(
      `📝 还需提升 ${progress.remainingImprovement.toFixed(2)}% 达到目标`,
    );
